...
export async function callAIAsk(
  ...
  // International Relations & Foreign Policy Domain
  if (upper.includes('INTERNATIONAL') || upper.includes('FOREIGN') || upper.includes('ECOWAS') || upper.includes('AFRICA') || upper.includes('AU') || upper.includes('DIPLOMACY') || upper.includes('DIASPORA') || upper.includes('SAHEL') || upper.includes('NIGER') || upper.includes('NIDCOM') || upper.includes('EMBASSY') || upper.includes('PASSPORT') || upper.includes('VISA') || upper.includes('TRADE') || upper.includes('BRICS') || upper.includes('UN') || upper.includes('AMERICA') || upper.includes('CHINA') || upper.includes('UK') || upper.includes('RELATION')) {
    ans = "Nigeria's foreign policy and international relations have historically been guided by an 'Afrocentric' doctrine — placing Africa at the center of national security, diplomacy, and trade. In today's shifting global order, Nigeria's international engagement is navigating high-stakes geopolitical developments across West Africa and the wider world.\n\nAt the regional level within ECOWAS, Nigeria serves as the economic and military anchor. Following recent political transitions in neighboring Niger, Mali, and Burkina Faso, regional dynamics have shifted as those nations formed the Alliance of Sahel States (AES). Nigeria's diplomatic priority has focused on constructive engagement to preserve regional integration, border security, and regional commerce.\n\nOn continental trade, Nigeria is a key player in the African Continental Free Trade Area (AfCFTA), a single market of 1.3 billion people, focusing on non-oil export diversification, customs automation, and port efficiency.\n\nGlobally, Nigeria maintains strategic partnerships:\n• With China: Infrastructure development (rail corridors, Lekki Deep Sea Port, and airport modernizations) alongside bilateral trade arrangements.\n• With the US, UK, and European Union: Counter-terrorism cooperation, maritime security in the Gulf of Guinea, and direct commercial investments.\n• Multilateral Engagement: Active leadership within the United Nations, African Union, Commonwealth, and partner engagement with the BRICS framework.\n\nDiaspora Impact: Over 15 million Nigerians abroad contribute over $20 billion annually in remittances. Through the Nigerians in Diaspora Commission (NiDCOM), engagement focuses on diaspora investment protection, passport issuance streamlining, and advocacy for voting rights.";
    links = [
  ...
  } else if (upper.includes('INFLATION') || upper.includes('ECONOMY') || upper.includes('NAIRA') || upper.includes('DOLLAR') || upper.includes('FX') || upper.includes('MINIMUM WAGE') || upper.includes('WAGE') || upper.includes('SALARY') || upper.includes('NLC') || upper.includes('TUC') || upper.includes('POVERTY') || upper.includes('TAX') || upper.includes('FIRS') || upper.includes('CBN')) {
    ...
    links = [
      ...
      { title: "Office of the National Security Adviser", url: "https://nsa.gov.ng", domain: "nsa.gov.ng" }
    ];
    ...
    links = [
      ...
      { title: "Nigerian Midstream & Downstream Petroleum Regulatory Authority", url: "https://nmdpra.gov.ng", domain: "nmdpra.gov.ng" },
